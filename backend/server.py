...
def _local_storage_fallback(relative_dir: Path, filename: str, data: bytes) -> str:
    """Fallback disque local pour le module Demandes d'offres."""
    safe_name = Path(filename).name or "fichier.bin"
    relative_dir.mkdir(parents=True, exist_ok=True)
    target = relative_dir / safe_name
...
def _file_response_headers(filename: str, inline: bool = True) -> dict:
    ...
    safe = (filename or "document").replace('"', "")
    encoded = quote(safe)
    return {
...
def _load_storage_bytes(storage_path: str) -> bytes:
    return _read_storage_bytes(storage_path)


def _resolve_download_filename(record: Optional[dict], fallback: str = "document.pdf", **_kwargs) -> str:
    if not record:
        return fallback
    return (
